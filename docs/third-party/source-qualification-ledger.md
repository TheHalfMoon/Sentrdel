...
| watchexec/process-wrap | `3d856eebd02799d025237134db51d05bbc4f1434` (`v9.1.0`; crates.io `process-wrap 9.1.0`) | NATIVE_DEP | upstream package declares `Apache-2.0 OR MIT` | `QUALIFIED_FOR_T027_UNIX_PROCESS_LIFECYCLE_CONTAINMENT` via PWQ-001; PWQ-001-D1 `CANDIDATE_PENDING_EXACT_HEAD_QUALIFICATION_AND_CANONICALIZATION` | exact pins/checksums; defaults disabled; `std` + `process-group` + `job-object`; `nix 0.31.3` privileged syscall/build surface recorded; the PWQ-001-D1 candidate enumerates the bounded macOS direct `nix` surface and deterministic lifecycle qualification without widening sandbox/network/provider authority |
...
features: process-wrap default-features=false; std; process-group; job-object. nix direct dependency default-features=false. On the PR candidate branch, PWQ-001-D1 records the complete direct `process` + `signal` safe API surface used by the bounded macOS post-reap group-absence proof and its test-only live-group canary. The effective resolved nix feature closure does not expand because process-wrap already enables `signal`, and nix `signal` implies `process`. PWQ-001-D1 remains candidate-only until protected-main canonicalization and post-merge governance verification.
...
security_notes: executable selection, argv, cwd confinement, env_clear allowlist, hard limits, network declaration admission, termination policy, and output interpretation remain Rust-owned Sentrdel authority. On the candidate branch, PWQ-001-D1 records exactly these direct macOS `nix` symbols/roles: `Errno::{ESRCH, EPERM}` for structured error identity; `Pid` with `from_raw` for the exact negative captured process-group identifier and `as_raw` in the test canary; `signal::kill` only for signal-zero probing; `signal::Signal` only as the `None::<Signal>` type used to express signal zero; and `getpgrp()` only under `cfg(test)` to obtain the canary's own known-live group. Initial macOS group-kill `ESRCH` and `EPERM` both require the post-reap proof; only probe `ESRCH` proves absence. Deterministic macOS tests additionally prove reap-before-probe ordering and fail-closed classification. The test-only lookup accepts no repository-controlled PID, and `Signal` naming grants no non-zero signal or arbitrary process-selection authority. No hostile-code sandbox, network, credential, provider, target-execution, policy, Finding, or Evidence authority is added. This candidate record does not claim canonical PWQ-001-D1 qualification before protected-main merge and post-merge governance verification.
...
modifications: no upstream source modified/copied; the PR candidate adds a private containment repair, deterministic lifecycle regressions, and the PWQ-001-D1 macOS post-reap absence proof/live-group canary.
qualified_by: Sentrdel dependency qualification review
...
qualification_delta_status: CANDIDATE_PENDING_EXACT_HEAD_QUALIFICATION_AND_CANONICALIZATION
```
