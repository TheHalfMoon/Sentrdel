...
security_notes: executable selection, argv, cwd confinement, env_clear allowlist, hard limits, network declaration admission, termination policy, and output interpretation remain Rust-owned Sentrdel authority. PWQ-001-D1 admits macOS safe `Pid` plus signal-zero probing of the exact spawned process group after an exited root is reaped; it also admits `nix::unistd::getpgrp()` only under `cfg(test)` so the macOS owning-seam canary can obtain its own known-live process-group ID and prove the probe never masks a live group. Initial macOS group-kill `ESRCH` and `EPERM` both require the post-reap proof; only `ESRCH` from the signal-zero probe proves absence. The test-only lookup accepts no repository-controlled PID and grants no production process-selection authority. No hostile-code sandbox, network, credential, provider, target-execution, policy, Finding, or Evidence authority is added.
