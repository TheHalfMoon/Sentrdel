...
model_path = Path("crates/sentrdel-review/src/regression/model.rs")
text = model_path.read_text()

old = """    TooManyCollectionItems {
        field: &'static str,
        count: usize,
        max: usize,
    },
...
old = """            Self::TooManyCollectionItems { field, count, max } => write!(
                formatter,
                "S1 collection {field} count {count} exceeds cap {max}"
            ),
            Self::DuplicateCoverageKey(value) => {
"""
new = """            Self::TooManyCollectionItems { field, count, max } => write!(
                formatter,
                "S1 collection {field} count {count} exceeds cap {max}"
            ),
...
old = """        validate_text(&draft.pair_id, "pair_id", limits)?;
        if draft.base_invariant_id.is_none() && draft.candidate_invariant_id.is_none() {
"""
new = """        validate_text(&draft.pair_id, "pair_id", limits)?;
...
marker = """fn validate_text(
    value: &str,
"""
helper = """fn enforce_total_input_bytes(
...
contract_path = Path("crates/sentrdel-review/tests/s1_phase1_contract.rs")
text = contract_path.read_text()
old = """    let forbidden = authority["forbidden_authority"]
        .as_object()
        .expect("forbidden authority object");
    assert!(forbidden.values().all(|value| value == false));
"""
new = """    let forbidden = authority["forbidden_authority"]
        .as_object()
        .expect("forbidden authority object");
...
"""
assert old in text
contract_path.write_text(text.replace(old, new, 1))
