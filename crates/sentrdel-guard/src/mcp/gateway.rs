...
//!
//! This module owns invocation normalization, pre-invocation policy, exact-scope
//! approval consumption, and forwarding/result bounds. The downstream stdio
//! transport remains a trusted adapter launched through T093's scrubbed child
//! environment boundary; repository or MCP payload bytes never select policy
//! authority or environment capabilities.
...
pub trait McpForwarder {
    /// Forward exactly one already-normalized invocation through the trusted
    /// stdio transport. Concrete process launch MUST use T093's
    /// `McpChildEnvironment`; this interface intentionally exposes no ambient
    /// environment or transport-selection input from the target payload.
...
impl fmt::Display for McpGatewayError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NameTooLarge { field, size, max } => {
                write!(formatter, "MCP {field} name is {size} bytes and exceeds cap {max}")
            }
            Self::Json(error) => write!(formatter, "MCP arguments cannot be serialized: {error}"),
            Self::ArgumentsTooLarge { size, max } => {
                write!(formatter, "MCP arguments are {size} bytes and exceed cap {max}")
            }
            ...
            Self::ApprovalScopeMismatch => formatter.write_str("MCP approval does not match invocation scope"),
            Self::ApprovalAlreadyConsumed => formatter.write_str("MCP scoped approval was already consumed"),
            Self::Denied => formatter.write_str("MCP invocation denied by preflight policy"),
            Self::Undecidable => formatter.write_str("MCP invocation policy was undecidable; failing closed"),
...
mod tests {
    ...
    #[test]
    fn allow_forwards_once_and_returns_bounded_result() {
    ...
    #[test]
    fn argument_and_result_caps_fail_closed() {
        ...
        assert!(matches!(
            McpInvocation::normalize("server", "tool", serde_json::json!({"large":"payload"}), limits),
        ...
        assert!(matches!(
            invoke_bounded(
                ...
                None,
                &mut forwarder,
                McpGatewayLimits { max_result_bytes: 4, ..McpGatewayLimits::default() },
            ),
    ...
    #[test]
    fn normalized_scope_binds_server_tool_and_arguments() {
