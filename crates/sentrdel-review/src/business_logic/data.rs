...
//!
//! Repository source is data only. This module observes a deliberately small static Supabase
//! fluent-API subset selected explicitly by the caller. It never executes a query or target code,
//! performs provider/network access, receives provider credentials, creates Findings, or claims
//! hosted/runtime database state. Provider-client authority is intentionally left unclassified.
...
impl DataExtraction {
    ...
    /// UNKNOWN values created only when a supported filter expression has no exact T012 value.
    #[must_use]
...
impl fmt::Display for DataExtractionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Structural(source) => write!(formatter, "data structural validation failed: {source}"),
            Self::Value(source) => write!(formatter, "data value-origin qualification failed: {source}"),
            ...
            Self::TooManyCoverageGaps { count, max } => {
                write!(formatter, "data coverage gap count {count} exceeds cap {max}")
            }
            ...
            Self::TooManySupportingValues { count, max } => {
                write!(formatter, "data supporting value count {count} exceeds cap {max}")
            }
...
/// Extract the frozen R3-T013 Supabase JavaScript/TypeScript operation subset.
///
/// `value_adapter` is used only to reuse canonical T012 value-origin qualification for filter and
/// field links. Selecting this data extractor does not prove that an arbitrary receiver is a live
/// Supabase client; `provider_client` remains `None` until later correlation tasks.
pub fn extract_supabase_data_operations(
    ...
    let digest = content_id("r3-data-source", &(path.as_str(), source_text)).map_err(ModelError::from)?;
    ...
    let mut builder = ExtractionBuilder::new(path, source_text, digest, limits, value_by_range);
    for node in &nodes {
        ...
        if is_direct_data_operation(method) {
            builder.observe_operation(*node, method, &nodes)?;
        }
    }
    builder.finish()
}
...
impl<'a> ExtractionBuilder<'a> {
    ...
    fn observe_operation(
        ...
        if receiver.kind() != "call_expression" || call_method(receiver, self.source) != Some("from") {
        ...
        let Some(resource_node) = from_args.first().copied() else {
            self.gap(DataCoverageGapReason::DynamicResource, receiver.start_byte(), receiver.end_byte())?;
            return Ok(());
        };
        ...
        if operation_kind == DataOperationKind::Read {
            ...
            if let Some(argument) = args.first().copied() {
                let (fields, is_partial) = self.mutation_field_set(argument, nodes)?;
    ...
    fn observe_rpc(
        ...
        let Some(name_node) = args.first().copied() else {
            self.gap(DataCoverageGapReason::DynamicRpcName, call.start_byte(), call.end_byte())?;
            return Ok(());
        };
        ...
        let start_string = call.start_byte().to_string();
        let end_string = call.end_byte().to_string();
        let operation_id = StableSemanticId::from_parts(
            "r3-data-operation",
            &[
                self.path.as_str(),
        ...
        self.operations.push(DataOperation::new(
            ...
            Some(rpc_name),
            None,
            vec![self.location(call.start_byte(), call.end_byte())?],
            if partial {
                CoverageState::Partial
            } else {
                CoverageState::Covered
    ...
    fn mutation_field_set(
        ...
        if self.is_broad_request_object(argument, nodes) {
            return Ok((
                FieldSet::new(
    ...
    fn binary_filter(
        ...
        let args = call_arguments(call);
        if args.len() < 2 {
            self.gap(DataCoverageGapReason::UnsupportedFilter, call.start_byte(), call.end_byte())?;
            return Ok((Vec::new(), true));
        }
        let field_node = args[0];
        let Some(field) = static_string(field_node, self.source).filter(|value| is_field_name(value)) else {
            self.gap(
    ...
    fn match_filters(
        ...
        for child in object.named_children(&mut cursor) {
            ...
            let Some(field) = static_property_name(key, self.source).filter(|value| is_field_name(value)) else {
                partial = true;
                self.gap(
                    DataCoverageGapReason::DynamicFilterField,
                    key.start_byte(),
    ...
    fn is_broad_request_object(
        &mut self,
        node: tree_sitter::Node<'_>,
        nodes: &[tree_sitter::Node<'_>],
    ) -> bool {
        let (root, request_like) = if node.kind() == "member_expression" {
            ...
            let Some(root) = node_text(object, self.source) else {
                return false;
            };
            (
                root,
                object.kind() == "identifier"
                    && node_text(property, self.source) == Some("body")
                    && matches!(root, "req" | "request"),
            )
        } else if node.kind() == "await_expression" {
            let Some(argument) = node.named_child(0) else {
                return false;
            };
            let Some((root, method)) = direct_member_call(argument, self.source) else {
                return false;
            };
            (root, method == "json" && matches!(root, "req" | "request"))
        } else {
            return false;
        };
        if !request_like {
        ...
        if !function_has_parameter(function, self.source, root) {
            return false;
        }
        if function_reassigns_name(function, nodes, self.source, root) {
            let _ = self.gap(
                DataCoverageGapReason::AmbiguousRequestObject,
                node.start_byte(),
                node.end_byte(),
            );
            return false;
...
fn is_direct_data_operation(method: &str) -> bool {
    matches!(method, "select" | "insert" | "update" | "upsert" | "delete" | "rpc")
}
...
fn direct_member_call<'a>(node: tree_sitter::Node<'_>, source: &'a str) -> Option<(&'a str, &'a str)> {
    if node.kind() != "call_expression" {
        return None;
    }
    ...
    Some((node_text(object, source)?, node_text(property, source)?))
}
...
fn function_has_parameter(function: tree_sitter::Node<'_>, source: &str, target: &str) -> bool {
