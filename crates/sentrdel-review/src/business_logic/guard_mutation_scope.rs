//! Conservative mutation qualification for bounded R3 typed guard extraction.
//!
//! The lower guard layers reject unsupported lexical binding ambiguity and direct identifier
//! reassignment. This final layer additionally invalidates supported alias-backed observations
//! when an object/array destructuring assignment can overwrite the alias. Ambiguous mutation
//! fails visible as unsupported coverage and never strengthens authorization evidence.
...
pub fn extract_guard_observations(
    ...
    let overwritten = collect_destructuring_reassignments(&nodes, source, &aliases);
    ...
    for guard in raw.guards() {
        if guard_mentions_overwritten_alias(guard, &nodes, source, &overwritten) {
            let Some(location) = guard.provenance().first() else {
                return Err(GuardExtractionError::ParseFailed(
                    "guard mutation qualifier returned empty provenance".to_owned(),
                ));
            };
            insert_gap(
                &mut gaps,
...
fn collect_destructuring_reassignments(
    nodes: &[tree_sitter::Node<'_>],
    source: &str,
    ...
    for node in nodes {
        ...
        let Some(left) = node.child_by_field_name("left").map(unwrap_expression) else {
            continue;
        };
        ...
        collect_assignment_bindings(left, source, &mut names);
        overwritten.extend(names.into_iter().filter(|name| aliases.contains(name)));
    }
    overwritten
}

fn collect_assignment_bindings(
    ...
    match node.kind() {
        ...
        "pair_pattern" => {
            if let Some(value) = node.child_by_field_name("value") {
                collect_assignment_bindings(value, source, names);
            }
        }
        "assignment_pattern" => {
            if let Some(left) = node.child_by_field_name("left") {
                collect_assignment_bindings(left, source, names);
            }
        }
        "rest_pattern" => {
            if let Some(argument) = node.named_child(0) {
                collect_assignment_bindings(argument, source, names);
            }
        }
        "object_pattern" | "array_pattern" => {
            let mut cursor = node.walk();
            for child in node.named_children(&mut cursor) {
                collect_assignment_bindings(child, source, names);
            }
        }
        _ => {}
    }
...
fn guard_mentions_overwritten_alias(
    ...
    for location in guard.provenance() {
        ...
        for node in nodes {
            ...
            if node_text(*node, source).is_some_and(|name| overwritten.contains(name)) {
                return true;
            }
        }
    }
...
fn parse_tree(
    ...
    parser.parse(source, None).ok_or_else(|| {
        GuardExtractionError::ParseFailed(
            "guard mutation-scope parser returned no syntax tree".to_owned(),
        )
    })
}
